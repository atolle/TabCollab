...
namespace TabRepository.Controllers
{
    ...
    public class FriendsController : Controller
    {
        ...
        public ActionResult FuzzySearch(string searchString)
        {
            ...
            {
                usernames.Add(new { username = user.UserName });
            }
        ...
        public ActionResult Search(string searchString, bool exact = false)
        {
            ...
            var users = exact == true
                        ...
                        orderby u.UserName
                        select new { u.Id, u.UserName, u.ImageFilePath, f.ActingUserId, f.User1Id, f.User2Id, Status = f.Status == null ? FriendStatus.None : f.Status }
                        :
                        from u in _context.Users
                        from f in _context.Friends.Where(f => (u.Id == f.User1Id && currentUserId == f.User2Id) || (u.Id == f.User2Id && currentUserId == f.User1Id)).DefaultIfEmpty()
                        where u.UserName.StartsWith(searchString)
                        orderby u.UserName
                        select new { u.Id, u.UserName, u.ImageFilePath, f.ActingUserId, f.User1Id, f.User2Id, Status = f.Status == null ? FriendStatus.None : f.Status };
